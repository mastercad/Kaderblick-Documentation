...
const output = new URL('../var/location-audit/', import.meta.url);
await mkdir(output, { recursive: true });
const browser = await chromium.launch({ headless: true, executablePath: '/usr/bin/google-chrome' });
const context = await browser.newContext({ viewport: { width: 1440, height: 1100 }, locale: 'de-DE', storageState: new URL('../var/ui-audit/club-admin/storage-state.json', import.meta.url).pathname });
...
await page.goto('https://demo.kaderblick.de/locations', { waitUntil: 'networkidle', timeout: 30000 });
...
const create = page.getByRole('button', { name: /Spielstätte.*anlegen|Neue Spielstätte/i }).first();
if (await create.isVisible().catch(() => false)) {
  await create.click();
  const dialog = page.getByRole('dialog').last();
  await dialog.waitFor();
  await waitForReady(page, dialog); await dialog.screenshot({ path: new URL('anlegen.png', output).pathname });
  await writeFile(new URL('anlegen.txt', output), await dialog.innerText());
}
await writeFile(new URL('fehler.txt', output), failures.join('\n'));
await browser.close();
console.log('Spielstätten als Vereinsadmin geprüft.');
